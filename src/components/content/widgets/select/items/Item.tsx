...
export const Item: React.FC<Props> = ({ item, isActive, onClick, closeSelect }) => {
  ...
  return <div
    className="cursor-pointer group/item flex items-center py-4 px-5 sm:py-5 sm:px-6 lg:py-5 lg:px-7"
    ...
    onClick={handleClick}
  >
    <img className={cn("w-5 h-5 transition-all", !isActive && 'opacity-60 group-hover/item:opacity-80')} src={item.img} />
    <p className={cn("text16-18 transition-all ml-2.5 md:ml-5", !isActive && 'text-dark-gray opacity-60 group-hover/item:text-light-text')}>{item.name}</p>
  </div>
};