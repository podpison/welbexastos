...
export const Item: React.FC<Props> = ({ answer, question, id, isOpen, onClick }) => {
  ...
  return <div className={cn("gradient-border-hover border-[1.5px] border-light-stroke transition-all cursor-pointer", isOpen && 'gradient-border')} onClick={() => onClick(id)}>
    <div
    className="group flex items-center justify-between transition-all gap-x-5 py-2.5 px-4 sm:py-4 sm:px-5 md:py-5 md:px-7"
    style={{background: isOpen ? 'linear-gradient(95.49deg, rgba(255, 246, 233, 0.1) 8.22%, rgba(26, 29, 35, 0.1) 75.4%)' : 'rgba(14, 16, 20, 0.6)'}}
    >
